        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            ...
            <Link href="/" className="flex items-center gap-3 group relative z-50">
              <div className={`rounded-2xl bg-gradient-to-br from-emerald-500 to-green-600 flex items-center justify-center transition-all duration-300 shadow-lg ${scrolled ? 'w-10 h-10' : 'w-12 h-12'
                } group-hover:scale-110 group-hover:rotate-6 group-hover:shadow-xl`}>
                <Leaf className="text-white" size={scrolled ? 20 : 24} />
              </div>
              <div className="flex flex-col">
                <span className={`font-black text-gray-900 transition-all duration-300 ${scrolled ? 'text-xl' : 'text-2xl'
                  }`}>
                  Claspuro
                </span>
              ...
              {links.map((link) => {
                ...
                return (
                  <Link
                    key={link.label}
                    href={link.href}
                    className={`relative text-sm lg:text-base font-bold transition-all duration-300 px-4 py-2.5 rounded-xl group ${isActive
                        ? "text-white"
            ...
            <button
              ...
              className={`md:hidden p-2.5 rounded-xl transition-all duration-300 ${isOpen
              ...
              {links.map((link, index) => {
                ...
                return (
                  <Link
                    key={link.label}
                    href={link.href}
                    className={`flex items-center justify-between px-5 py-3.5 rounded-xl font-bold transition-all duration-300 ${isActive
